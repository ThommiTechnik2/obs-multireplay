...
void AspectBox::relayout()
{
	...
	// sides is not proportional), and an earlier harness's "every picture is
	// 16:9" check caught exactly that: 172x95 where 96 was wanted.
	...
	: QWidget(parent), caption_(caption)
{
	...
	// put a moc'd type in a header kept deliberately moc-free - so a name is
	// how a section is found from outside, exactly as the camera tiles are.
...
void KeyBlock::apply()
{
	...
	// TwoPanelStrip::addToMarca/setReviewGrid — the pre-redesign strip
	...
	for (const QVector<Cell> &row : s) {
		...
		for (const Cell &cell : row) {
			...
			// which is exactly what the harness drew the first time
			// it ran.
	...
	if (stretchFrom_ >= 0) {
		...
		// content, e.g. inside a lane-aligned strip.)
		if (centreH) {
...
// ---------------------------------------------------------------------------
// repinKeys — give a key back the fixed height a stylesheet polish stole
// ---------------------------------------------------------------------------
